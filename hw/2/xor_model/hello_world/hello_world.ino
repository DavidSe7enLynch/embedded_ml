...
void setup() {
  ...
  if (model_type == 0) {
    ...
    model = tflite::GetModel(xor_model);
    pinMode(D2, INPUT);
    pinMode(D4, INPUT);
    pinMode(LED_BUILTIN, OUTPUT);
  } else {
    MicroPrintf("Wrong Model Type\n");
    return;
  }
...
void xor_model_loop() {
  ...
  MicroPrintf("x0 = %d, x1 = %d, x0_quantized = %d, x1_quantized = %d, y_quantized = %d, y = %f\n", x0, x1, x0_quantized, x1_quantized, y_quantized, y);  
}
